...
def make_X(n=100):
    m=n+1
    x = np.arange(m).reshape(-1,1) * np.ones(m)
    y = x.T
    x = np.ravel(x)
    y = np.ravel(y)
    X = np.array([x,y]).T
...
def make_target(X):
    x,y = X[:,0],X[:,1]
    return np.where((x*x<100*y) & (x+y<100),1,0 )


def real_labels(X,target):
    plt.figure()
    plt.scatter(*np.hsplit(X[target == 1],2), c="r")
    plt.scatter(*np.hsplit(X[target == 0],2), c="b")
...
def prd_labels(X,target,clf):
    ...
    plt.scatter(*np.hsplit(X[t_prd == 1], 2), c="r")
    plt.scatter(*np.hsplit(X[t_prd == 0], 2), c="b")
...
def main():
    ...
    clf_list = [nn.MLPClassifier(hidden_layer_sizes=(100,)),
