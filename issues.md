...
- 模型调优 空间很大
    - clf vs reg
        - 试验表明两者参数类似，clf效果更好。
        - 理论分析，clf更合理，也更适用本场景
            - 优点：因为y = 0.6,0.4对交易没有区别，y= 0.05,-0.05对交易也没区别，clf使得模型只专注于我们关注的对象（是否达到涨幅阈值），而不关心精确值（很难预测）
            - 缺点：阈值定义需要人工定义——实验与数据分布、交易模式。
	- 过拟合 vs 欠拟合
	- min_gain/gamma =0 and 过多warning no positive gain -> 过拟合
	- max_depth 影响最大？ max_depth 与 min_child_sample互相强影响
	- n_estimator num_leaves 影响中等 
	- min_child_sample 与 样本量有关 其他参数与样本量无关


- 修改模型
	- 使用sample_weight：一定程度上模拟交易收益（最粗糙）
	- 自定义loss函数：近似模拟交易（较粗糙）
...
- y值transform与验证
	- y根据简化的交易模式计算近似revenue（粗糙）
	- 每个y跑一次交易模式(独立跑)，生成revenue（较准确）
		- 考虑了交易策略(止损止盈平仓)，更准
		- 没考虑不同股票竞争买入与进入时点
		- 缺点：1. 慢 2. 
...
- 模型验证评估方法
	- 回测：耗时但更接近实盘，但有偶然性问题。
	    - 尤其是大量出现资金不足以建新仓位的情况，选股建仓可能有偶然性
	    - 或者某些交易模式操作次数过少，存在偶然性。
	    - 解决办法：
	        - 尽量减少操作的随机性与不确定性。
	        - 多次回测：
	            - 股票池随机+人为抽样
